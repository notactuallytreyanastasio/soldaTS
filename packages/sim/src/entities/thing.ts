...
// ===========================================================================
// Per-style skeleton physics config — PORT: CreateThing case (Things.pas:147).
// VDamping / Gravity are written per-skeleton in Pascal; here we record them per
// thing-style so updateThing can apply them onto the shared system per thing.
// ===========================================================================

interface ThingStyleConfig {
  vDamping: number;
  gravity: number;
  radius: number;
  timeOut: number;
...
function styleConfig(style: number): ThingStyleConfig {
  const S = ObjectStyle;
  switch (style) {
...
const thingGravity: number[] = new Array<number>(MAX_THINGS + 1).fill(0);
const thingVDamping: number[] = new Array<number>(MAX_THINGS + 1).fill(0);
 ...
 */
export function configureThingParts(parts: ParticleSystem): void {
 ...
 */
function buildThingSkeleton(parts: ParticleSystem, thingIndex: number): void {
 ...
 */
export function killThing(world: World, thingIndex: number): void {
...
function checkOutOfBounds(world: World, thingIndex: number): void {
  const map = world.map;
  const parts = world.thingParts;
  if (map === null || parts === null) {
    return;
  ...
  for (let i = 1; i <= THING_SKELETON_PARTS; i++) {
    ...
    if (Math.abs(px) > bound || Math.abs(py) > bound) {
      ...
      if (
        (style >= ObjectStyle.ALPHA_FLAG && style <= ObjectStyle.POINTMATCH_FLAG) ||
 ...
 */
function respawnThing(world: World, thingIndex: number): void {
  const parts = world.thingParts;
  const thing = world.things[thingIndex];
  ...
  const a: Vec2 = vec2(parts.posX[partOf(thingIndex, 1)]!, parts.posY[partOf(thingIndex, 1)]!);
 ...
 */
function checkSpriteCollision(world: World, thingIndex: number): number {
  const parts = world.thingParts;
  ...
  for (let j = 1; j <= MAX_SPRITES; j++) {
    ...
    if (dist < radius && dist < closestDist) {
      ...
      const fullHealthMedikit =
        thing.style === ObjectStyle.MEDICAL_KIT && sprite.health >= 150; // STARTHEALTH
      const flagDuringCeaseFire =
        thing.style < ObjectStyle.USSOCOM && sprite.ceaseFireCounter > 0;
 ...
 * PORT: shared/mechanics/Things.pas:774-938.
 */
function checkBaseAndTouchdown(world: World, thingIndex: number): void {
  const parts = world.thingParts;
  const thing = world.things[thingIndex];
  if (parts === null || thing === undefined) {
    return;
 ...
 */
export function updateThing(world: World, thingIndex: number): void {
  const parts = world.thingParts;
  const spriteParts = world.spriteParts;
  const thing = world.things[thingIndex];
