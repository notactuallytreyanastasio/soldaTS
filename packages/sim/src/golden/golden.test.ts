...
describe(`golden harness (STRICT_F32=${STRICT_F32})`, () => {
  ...
  it('same scenario run twice yields byte-identical traces (determinism)', () => {
    const a: GoldenTrace = freeFallScenario(TICKS).run();
    const b: GoldenTrace = freeFallScenario(TICKS).run();
  ...
  it('compareTraces flags a deliberately perturbed trace at the right tick', () => {
    ...
    const perturbed: GoldenTrace = JSON.parse(JSON.stringify(base)) as GoldenTrace;
    ...
    const structural: GoldenTrace = JSON.parse(JSON.stringify(base)) as GoldenTrace;
