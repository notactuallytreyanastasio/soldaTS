...
export const STARTHEALTH = 150; // Constants.pas:75 — STARTHEALTH = 150
export const BRUTALDEATHHEALTH = -400; // Constants.pas:77 — BRUTALDEATHHEALTH = -400
// PORT: shared/mechanics/Bullets.pas:1617-1622 — hitbox modifier by body part.
//   Where <= 4  -> ModifierLegs
//   Where <= 11 -> ModifierChest
//   else        -> ModifierHead
export function hitboxModifier(gun, where) {
    if (where <= 4) {
 ...
 */
export function applyBulletDamage(world, bulletIndex, spriteIndex, hitPart, gun) {
    ...
    let velocity = bullet.velocityPrev;
    ...
    if (world.onBulletHit !== null && bp !== null) {
        world.onBulletHit(spriteIndex, bp.posX[bullet.num] ?? 0, bp.posY[bullet.num] ?? 0, velocity.x, velocity.y, hm, !wasDead && sprite.deadMeat);
    }
    return hm;
}
/**
 ...
 */
export function applyHealthHit(world, spriteIndex, amount, bulletIndex) {
    const sprite = world.sprites[spriteIndex];
    if (sprite === undefined) {
        return 0;
    }
