...
import { RUNSPEED, RUNSPEEDUP, FLYSPEED, JUMPSPEED, JUMPDIRSPEED, JETSPEED, SURFACECOEFX, SURFACECOEFY, STANDSURFACECOEFX, STANDSURFACECOEFY, CROUCHMOVESURFACECOEFX, CROUCHMOVESURFACECOEFY, DEFAULT_GRAVITY, } from '../constants';
import { flatGroundCollision, SLIDELIMIT, SPRITE_COLLISION_POINTS, } from '../map/collision';
 ...
 */
export function applyControl(sprite, control, spriteParts) {
    ...
    const jetting = control.jetpack && sprite.jetsCount > 0 && sprite.position !== POS_PRONE;
    applyJetpack(sprite, control, spriteParts);
    ...
    if (control.right && !control.left) {
        if (sprite.onGround) {
            ...
            spriteParts.forceY[num] = f(Math.min(spriteParts.forceY[num] ?? 0, -RUNSPEEDUP));
        }
        else {
 ...
 */
export function applyJetpack(sprite, control, spriteParts) {
    ...
    if (sprite.onGround) {
        ...
        spriteParts.forceY[num] = f(Math.min(spriteParts.forceY[num] ?? 0, -JET_GROUND_THRUST));
    }
 ...
 */
export function configureSpriteParts(spriteParts) {
 ...
 * Sprites.pas:2786.
 */
export function applySurfaceFriction(sprite, spriteParts, stepY) {
    const num = sprite.num;
    ...
    if (slow) {
        ...
        spriteParts.forceX[num] = f((spriteParts.forceX[num] ?? 0) - (spriteParts.velocityX[num] ?? 0));
        return;
    }
 ...
 */
export function clampVelocity(sprite, spriteParts) {
    const num = sprite.num;
    let vx = spriteParts.velocityX[num] ?? 0;
    let vy = spriteParts.velocityY[num] ?? 0;
    if (vx > MAX_VELOCITY)
        vx = MAX_VELOCITY; // Sprites.pas:1414-1415
    if (vx < -MAX_VELOCITY)
        vx = -MAX_VELOCITY; // Sprites.pas:1416-1417
    if (vy > MAX_VELOCITY)
        vy = MAX_VELOCITY; // Sprites.pas:1418-1419
    if (vy < -MAX_VELOCITY)
        vy = -MAX_VELOCITY; // Sprites.pas:1420-1421
 ...
 */
export function applyGravityAndFriction(sprite, spriteParts, floorY) {
    const num = sprite.num;
    ...
    // zero look-ahead.)
    const onGround = flatGroundCollision(spriteParts.posY[num] ?? 0, 0, floorY);
 ...
 */
export function updateSpriteMovement(world, spriteIndex, floorY) {
 ...
 */
export function updateSpriteMovementMap(world, spriteIndex, radius) {
    const sprite = world.sprites[spriteIndex];
    const spriteParts = world.spriteParts;
    if (sprite === undefined || spriteParts === null) {
        return;
 ...
 */
export function resolveParticleMapCollision(parts, num, col) {
 ...
 */
export function collideSpriteAgainstMap(world, spriteIndex, radius) {
    const sprite = world.sprites[spriteIndex];
    ...
    const accept = (polyType) => !isBackground(polyType) && !isOnlyBullets(polyType);
