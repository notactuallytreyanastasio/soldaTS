...
import { POLY_TYPE_ONLY_PLAYER, POLY_TYPE_DOESNT, POLY_TYPE_ONLY_FLAGGERS, POLY_TYPE_NOT_FLAGGERS, POLY_TYPE_BACKGROUND, POLY_TYPE_BACKGROUND_TRANSITION, } from '../map/polymap';
 ...
 */
export function configureBulletParts(bulletParts) {
 ...
 */
export function spawnBullet(world, args) {
    ...
    let sPos = { x: args.pos.x, y: args.pos.y };
 ...
 */
export function bulletCollidesWithPoly(polyType) {
 ...
 */
export function checkBulletMapCollision(world, bulletIndex) {
    const map = world.map;
    const bp = world.bulletParts;
 ...
 */
export function checkSpriteCollision(world, bulletIndex) {
    const bp = world.bulletParts;
    ...
    const start = { x: bp.posX[bullet.num] ?? 0, y: bp.posY[bullet.num] ?? 0 };
    const vel = {
        x: bp.velocityX[bullet.num] ?? 0,
        y: bp.velocityY[bullet.num] ?? 0,
    };
    const end = { x: f(start.x + vel.x), y: f(start.y + vel.y) };
    let best = null;
    ...
    for (let j = 1; j <= world.sprites.length - 1; j++) {
        ...
        const center = { x: sp.posX[j] ?? 0, y: sp.posY[j] ?? 0 };
 ...
 */
function lineCircleClosest(start, end, center, radius) {
 ...
 */
export function killBullet(world, bulletIndex) {
    const bullet = world.bullets[bulletIndex];
    if (bullet === undefined) {
        return;
    }
 ...
 */
export function updateBullet(world, bulletIndex, gun) {
    const bp = world.bulletParts;
    const bullet = world.bullets[bulletIndex];
 ...
 */
export function isHitscanStyle(style) {
    return (style === BulletStyle.PLAIN ||
