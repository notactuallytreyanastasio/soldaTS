 ...
 */
export function isDeadly(polyType) {
    return (polyType === POLY_TYPE_DEADLY ||
...
export class PolyMap {
    ...
    static pointInPoly(p, poly) {
        ...
        const pBc = f(f(c.x - b.x) * f(p.y - b.y)) - f(f(c.y - b.y) * f(p.x - b.x)) > 0;
        if (pBc !== pAb) {
            return false;
        }
        return true;
     ...
     */
    pointInPolyEdges(x, y, polyIndex) {
        const poly = this.polys[polyIndex];
        if (poly === undefined) {
            return false;
        }
     ...
     */
    closestPerpendicular(polyIndex, pos) {
        const poly = this.polys[polyIndex];
        if (poly === undefined) {
     ...
     */
    collidePointPushout(pos, accept) {
     ...
     */
    collideCircle(center, radius, accept) {
...
// Internal helpers
// ---------------------------------------------------------------------------
const EMPTY_SECTOR = Object.freeze([]);
/**
