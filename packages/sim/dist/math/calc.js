...
export function isLineIntersectingCircle(line1, line2, circleCenter, radius) {
    ...
    const c1 = f(f(ccy * ccy) - f(radius * radius) + f(ccx * ccx) - f(2 * f(b * ccy)) + f(b * b));
...
export function lineCircleCollision(startPoint, endPoint, circleCenter, radius) {
...
export function pointLineDistance(p1, p2, p3) {
    const dx = f(p2.x - p1.x);
    const dy = f(p2.y - p1.y);
    // PORT: shared/Calc.pas:204-205.
    const u = f(f(f(f(p3.x - p1.x) * dx) + f(f(p3.y - p1.y) * dy)) / f(f(dx * dx) + f(dy * dy)));
...
export function angle2Points(p1, p2) {
    const dx = f(p2.x - p1.x);
    if (dx !== 0) {
        const dy = f(p2.y - p1.y);
        if (p1.x > p2.x) {
...
export function distanceV(p1, p2) {
...
export function sqrDistV(p1, p2) {
...
export function greaterPowerOf2(n) {
...
export function roundFair(value) {
