...
import { ChatChannel, HandshakeResult, Posture, PROTOCOL_VERSION, } from "./messages.js";
...
const SpriteSnapshotTag = { full: 0, delta: 1 };
const HandshakeTag = { hello: 0, welcome: 1 };
// ---------------------------------------------------------------------------
// Writer
// ---------------------------------------------------------------------------
class Writer {
...
class Reader {
    ...
    need(n) {
        if (this.pos + n > this.len) {
            throw new DecodeError("truncated", `buffer truncated: need ${n} byte(s) at offset ${this.pos}, have ${this.len - this.pos}`);
        }
    }
    get done() {
    ...
    expectEnd() {
        if (!this.done) {
            throw new DecodeError("invalid-value", `trailing ${this.len - this.pos} byte(s) after payload`);
        }
    }
}
// ---------------------------------------------------------------------------
...
function writeMouseAim(w, a) {
    w.svarint(a.x);
    w.svarint(a.y);
}
function readMouseAim(r) {
    const x = r.svarint();
    const y = r.svarint();
    return { x, y };
}
function writePosture(w, p) {
    w.u8(p);
}
function readPosture(r) {
    const v = r.u8();
...
function readChatChannel(r) {
    const v = r.u8();
    if (v !== ChatChannel.Command &&
...
function writeVec2Array(w, arr) {
    w.uvarint(arr.length);
    for (const v of arr)
        writeVec2(w, v);
}
function readVec2Array(r) {
    const n = r.uvarint();
    const out = [];
    for (let i = 0; i < n; i++)
        out.push(readVec2(r));
...
function writeInputFrame(w, f) {
    w.svarint(f.clientTick);
    writeButtons(w, f.buttons);
    writeMouseAim(w, f.aim);
    writePosture(w, f.posture);
    let mask = 0;
    if (f.predictedPos !== undefined)
        mask |= 0x1;
    if (f.predictedVel !== undefined)
        mask |= 0x2;
    w.u8(mask);
    if (f.predictedPos !== undefined)
        writeVec2(w, f.predictedPos);
    if (f.predictedVel !== undefined)
        writeVec2(w, f.predictedVel);
}
function readInputFrame(r) {
    const clientTick = r.svarint();
    const buttons = readButtons(r);
    const aim = readMouseAim(r);
    const posture = readPosture(r);
    const mask = r.u8();
    const base = { clientTick, buttons, aim, posture };
    if (mask & 0x1)
        base.predictedPos = readVec2(r);
    if (mask & 0x2)
        base.predictedVel = readVec2(r);
    return base;
}
// --- SpriteSnapshot (full | delta) ---
function writeSpriteSnapshotFull(w, s) {
    w.uvarint(s.num);
...
function readSpriteSnapshotFull(r) {
    const num = r.uvarint();
...
function writeSpriteSnapshotDelta(w, s) {
    w.uvarint(s.num);
    w.svarint(s.serverTick);
    let mask = 0;
    if (s.pos !== undefined)
    ...
    if (s.wearHelmet !== undefined)
        w.u8(s.wearHelmet ? 1 : 0);
}
function readSpriteSnapshotDelta(r) {
    const num = r.uvarint();
    const serverTick = r.svarint();
    const mask = r.uvarint();
    const out = { kind: "delta", num, serverTick };
    if (mask & 0x001)
        out.pos = readVec2(r);
    if (mask & 0x002)
        out.velocity = readVec2(r);
    if (mask & 0x004)
        out.aim = readMouseAim(r);
    if (mask & 0x008)
        out.posture = readPosture(r);
    if (mask & 0x010)
        out.buttons = readButtons(r);
    if (mask & 0x020)
        out.health = r.f32();
    if (mask & 0x040) {
        const weaponNum = r.uvarint();
    ...
    if (mask & 0x080)
        out.wearHelmet = r.u8() !== 0;
    return out;
}
function writeSpriteSnapshot(w, snap) {
    if (snap.kind === "full") {
...
function readSpriteSnapshot(r) {
    const tag = r.u8();
    if (tag === SpriteSnapshotTag.full)
        return readSpriteSnapshotFull(r);
    if (tag === SpriteSnapshotTag.delta)
        return readSpriteSnapshotDelta(r);
    throw new DecodeError("unknown-tag", `unknown SpriteSnapshot tag ${tag}`);
}
// --- SkeletonSnapshot ---
function writeSkeletonSnapshot(w, s) {
    w.uvarint(s.num);
...
function readSkeletonSnapshot(r) {
    const num = r.uvarint();
...
function writeThingSnapshot(w, s) {
    w.uvarint(s.num);
    w.uvarint(s.owner);
    w.uvarint(s.style);
...
function readThingSnapshot(r) {
    ...
    const out = { num, owner, style, holdingSprite, pos, oldPos };
    if (hasTimeout)
        out.timeout = r.svarint();
    return out;
}
// --- Heartbeat ---
function writeScoreboardEntry(w, e) {
    w.uvarint(e.num);
    w.u8(e.active ? 1 : 0);
    w.uvarint(e.team);
    w.uvarint(e.kills);
...
function readScoreboardEntry(r) {
    const num = r.uvarint();
    const active = r.u8() !== 0;
...
function writeHeartbeat(w, h) {
    w.uvarint(h.mapId);
    w.uvarint(h.teamScore.length);
    for (const s of h.teamScore)
        w.uvarint(s);
    w.uvarint(h.players.length);
    for (const p of h.players)
        writeScoreboardEntry(w, p);
}
function readHeartbeat(r) {
    const mapId = r.uvarint();
    const scoreCount = r.uvarint();
    const teamScore = [];
    for (let i = 0; i < scoreCount; i++)
        teamScore.push(r.uvarint());
    const playerCount = r.uvarint();
    const players = [];
    for (let i = 0; i < playerCount; i++)
        players.push(readScoreboardEntry(r));
    return { mapId, teamScore, players };
}
// --- Chat ---
function writeChat(w, c) {
...
function writeHandshakeHello(w, h) {
...
function writeHandshakeWelcome(w, h) {
    w.uvarint(h.result);
    w.uvarint(h.protocolVersion);
    let mask = 0;
    if (h.yourNum !== undefined)
        mask |= 0x1;
    if (h.mapName !== undefined)
        mask |= 0x2;
    if (h.serverTick !== undefined)
        mask |= 0x4;
    ...
    if (h.mapName !== undefined)
        w.string(h.mapName);
    if (h.serverTick !== undefined)
        w.svarint(h.serverTick);
    if (h.reason !== undefined)
        w.string(h.reason);
}
function readHandshakeWelcome(r) {
    const result = readHandshakeResult(r);
    const protocolVersion = r.uvarint();
    const mask = r.u8();
    const out = { kind: "welcome", result, protocolVersion };
    if (mask & 0x1)
        out.yourNum = r.uvarint();
    if (mask & 0x2)
        out.mapName = r.string();
    if (mask & 0x4)
        out.serverTick = r.svarint();
    if (mask & 0x8)
        out.reason = r.string();
    return out;
}
function writeHandshake(w, h) {
    if (h.kind === "hello") {
...
function readHandshake(r) {
    const tag = r.u8();
    if (tag === HandshakeTag.hello)
        return readHandshakeHello(r);
    if (tag === HandshakeTag.welcome)
        return readHandshakeWelcome(r);
...
export function decodeMessage(buf) {
    ...
    if (version !== PROTOCOL_VERSION) {
        throw new DecodeError("version-mismatch", `protocol version mismatch: got ${version}, expected ${PROTOCOL_VERSION}`);
    }
    const tag = r.u8();
    let msg;
    switch (tag) {
