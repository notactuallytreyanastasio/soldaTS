...
export interface RunManifest {
  ...
  gitRev: string; // `git rev-parse HEAD` (trimmed); 'unknown' on failure
  map: 'Skyreach';
  botCount: number;
  roundTicks: number;
  maxTicks: number;
...
export interface RunSummary {
  ...
  standings: {
    red: { engine: string; wins: number; kills: number; deaths: number; dominance: number };
    blue: { engine: string; wins: number; kills: number; deaths: number; dominance: number };
    draws: number;
...
export function makeRunId(teamA: string, teamB: string, suffix?: string, now?: Date): string {
...
export function buildManifest(args: ManifestArgs): RunManifest {
  const first = args.results[0];
  if (first === undefined) throw new Error('buildManifest: no match results');
  const teamManifest = (side: 0 | 1): TeamManifest => ({
    team: (side + 1) as 1 | 2,
...
export function buildSummary(results: readonly MatchResult[]): RunSummary {
  const matches: RunSummary['matches'] = [];
  const teamTotals = {
  ...
  const botTotals = new Map<
    number,
    { name: string; engine: string; team: number; kills: number; deaths: number; shots: number; hits: number }
  >();
  ...
  results.forEach((r, i) => {
    ...
    matches.push({
      ...
      seed: r.seed,
      ticks: r.ticks,
      winnerTeam: (round?.winnerTeam ?? 0) as 0 | 1 | 2,
    ...
    if (round === null || round.winnerTeam === 0) draws += 1;
    else if (round.winnerTeam === 1) teamTotals.red.wins += 1;
    else teamTotals.blue.wins += 1;

    for (const bot of r.bots) {
      const side = bot.team === 1 ? teamTotals.red : teamTotals.blue;
      if (side.engine === '') side.engine = bot.engine;
  ...
  const winner =
    teamTotals.red.wins > teamTotals.blue.wins
...
export function writeRun(
  ...
  results.forEach((r, i) => {
    const n = i + 1;
    fs.writeFileSync(
      path.join(dir, `match-${n}.replay.jsonl.gz`),
      zlib.gzipSync(Buffer.from(r.replayJsonl)),
    );
    ...
    fs.writeFileSync(
      path.join(dir, `match-${n}.telemetry.json.gz`),
      zlib.gzipSync(Buffer.from(JSON.stringify(r.telemetry, null, 2))),
    );
