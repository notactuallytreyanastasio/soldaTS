 ...
 */
export function parseTeams(teamsOpt, positionals) {
    let a;
    let b;
...
export function parseTweaks(list) {
    const out = {};
...
export function parseWildcard(raw) {
    if (raw === undefined)
        return 'chance';
    if (!WILDCARD_MODES.includes(raw)) {
...
export function parseSweep(spec) {
    if (spec === undefined)
        return null;
    ...
    return { team: m[1], key: m[2].trim(), values };
}
/**
