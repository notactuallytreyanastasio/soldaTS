...
import { buildRunPlans, parseSweep, parseTeams, parseTweaks, parseWildcard, WILDCARDS, WILDCARD_MODES, } from './cliArgs';
import { runMatch } from './runner';
import { buildManifest, makeRunId, writeRun } from './store';
import { buildWatchUrl, validateCard } from './fighterCard';
...
function main() {
    ...
    const intArg = (raw, name, fallback) => {
        if (raw === undefined)
            return fallback;
        const n = Number(raw);
    ...
    try {
        ...
        if (!VARIANTS.some((v) => v.name === variant)) {
            throw new Error(`unknown variant '${variant}' (known: ${VARIANTS.map((v) => v.name).join(', ')})`);
        }
        const shared = {
        ...
        if (teams !== null) {
            ...
            for (let i = 0; i < roster.length; i++) {
                for (let j = i + 1; j < roster.length; j++) {
                    plans.push(...buildRunPlans({ ...shared, teams: [roster[i], roster[j]], matches }));
                }
            }
            console.log(`LEAGUE — no --teams given: round-robin over ${roster.length} engines ` +
    ...
    for (const plan of plans) {
        ...
        for (let k = 0; k < plan.matches; k++) {
            ...
            console.log(`[${plan.label}] match ${k + 1}/${plan.matches}  seed ${seed}  ${verdict}` +
        ...
        const dir = writeRun(outDir, runId, buildManifest({
            runId,
            teams: plan.teams,
            results,
        ...
        for (const r of results) {
            if (r.round?.winnerTeam === 1)
                wins.red += 1;
            else if (r.round?.winnerTeam === 2)
                wins.blue += 1;
        ...
        console.log(`[${plan.label}] standings: ${plan.teams[0].engine} ${wins.red}W ${kills.red}K dom ${dom.red.toFixed(1)}` +
    ...
    if (outcomes.length > 1) {
        ...
        for (const o of outcomes) {
            ...
            console.log(`  ${o.plan.label.padEnd(20)} red ${redWins}W  blue ${blueWins}W  red ${redKills}K  blue ${blueKills}K`);
        }
    }
}
main();
/** The Claude Arena fight runner: two fighter cards in, dataset + watch URL out. */
function fight(cardPaths, values) {
    ...
    const outDir = values.out ?? fileURLToPath(new URL('../../../datasets', import.meta.url));
    console.log(`CLAUDE ARENA — ${a.coach} (${a.engine}) vs ${b.coach} (${b.engine}) · ` +
        `${matches} match(es) · ${roundSecs}s rounds · arena #${arenaSeed}` +
        ` · wildcard ${wildcard}`);
    if (a.rationale !== undefined)
        console.log(`  ${a.coach}: "${a.rationale}"`);
    if (b.rationale !== undefined)
        console.log(`  ${b.coach}: "${b.rationale}"`);
    ...
    for (let k = 0; k < matches; k++) {
        ...
        console.log(`  match ${k + 1}/${matches}  seed ${seed}  ${verdict}  ${wallSecs.toFixed(1)}s`);
        live.matchDone({
            n: k + 1,
            seed,
