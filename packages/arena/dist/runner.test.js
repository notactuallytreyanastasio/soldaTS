...
describe('runMatch', () => {
    ...
    it('replay rows have consistent bots/teams/engines and non-decreasing ticks', () => {
        ...
        for (const row of rows) {
            ...
            if (seen === undefined)
                teamOf.set(row.bot, row.team);
            else
                expect(row.team).toBe(seen);
    ...
    it('records well-formed events that agree with the telemetry', () => {
        ...
        for (const e of result.events) {
            ...
            if (e.type === 'shot')
                expect(e.bot).toBeGreaterThan(1);
            if (e.type === 'hit') {
    ...
    it('throws on mirror matches and unknown engines', () => {
        expect(() => runMatch({ seed: 1, teams: [{ engine: 'pilot' }, { engine: 'pilot' }], roundTicks: 60 })).toThrow(/mirror matches/);
        expect(() => runMatch({ seed: 1, teams: [{ engine: 'pilot' }, { engine: 'nope' }], roundTicks: 60 })).toThrow(/unknown engine 'nope'/);
    });
});
