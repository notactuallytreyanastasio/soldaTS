...
import { createNeuralEngineWithWeights, registerEngine, } from '@soldat/client/headless';
...
export { NEURAL_SHIPPED_NET } from '@soldat/client/headless';
// ---------------------------------------------------------------------------
...
export function paramCount(dims) {
    let n = 0;
...
export function flattenNet(net) {
    ...
    for (let l = 0; l < net.dims.length - 1; l++) {
        for (const v of net.weights[l] ?? [])
            flat[o++] = v;
        for (const v of net.biases[l] ?? [])
            flat[o++] = v;
    }
    if (o !== flat.length)
        throw new Error(`flattenNet: ${o} values for ${flat.length} params`);
    return flat;
}
export function unflattenNet(dims, flat) {
...
export function samplePerturbations(rng, dim, pairs) {
    const eps = [];
...
export function rankShape(fitnesses) {
 ...
 */
export function esUpdate(mean, eps, fitPlus, fitMinus, opts) {
    const n = eps.length;
...
export function makeCheckpoint(gen, dims, mean, pastSelves, meanFitness, bestFitness) {
    return {
...
export function parseCheckpoint(json) {
...
export function registerNeuralNet(id, net) {
