...
import { buildManifest, buildSummary, makeRunId, writeRun, } from './store';
describe('makeRunId', () => {
    it('formats YYYYMMDD-HHMMSS-<a>-vs-<b> in UTC from the injected clock', () => {
        const now = new Date(Date.UTC(2026, 5, 10, 15, 30, 5));
        expect(makeRunId('pilot', 'reaper', undefined, now)).toBe('20260610-153005-pilot-vs-reaper');
    });
    it('appends the suffix when given', () => {
        const now = new Date(Date.UTC(2026, 0, 2, 3, 4, 5));
        expect(makeRunId('pilot', 'reaper', 'RANGE_MAX-420', now)).toBe('20260102-030405-pilot-vs-reaper-RANGE_MAX-420');
    });
});
describe('writeRun + buildManifest + buildSummary', () => {
    ...
    beforeAll(() => {
        ...
        results = [5, 6].map((seed) => runMatch({ seed, teams, botCount: 4, variant: 'high-octane', roundTicks: 600 }));
    ...
    it('summary totals are consistent with the match results', () => {
        ...
        for (const r of results) {
            if (r.round === null || r.round.winnerTeam === 0)
                draws += 1;
            else if (r.round.winnerTeam === 1)
                redWins += 1;
            else
                blueWins += 1;
        }
        ...
        const expectWinner = redWins > blueWins
            ? 'pilot'
            : blueWins > redWins
                ? 'reaper'
