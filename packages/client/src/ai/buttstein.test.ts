// "buttstein" engine (buttstein.ts) — the fourth student: v3 senses (exact
// threat data, own spray heat) over the prodigy's spine. Tests pin the
// weight-independent invariants (thresholds, aim magnitude, empty-mag rule,
// roam, guards), the memory protocol's guaranteed gap identity, and — against
// the committed weight set — that the v3-specific heat sense and the bullet
// threat actually reach the policy.

import { describe, it, expect } from 'vitest';
import type { World } from '@soldat/sim';
import type { BotEngineContext } from './engine';
import { createButtsteinEngine, BUTTSTEIN_DEFAULTS } from './buttstein';
...
describe('createButtsteinEngine', () => {
  it("returns id 'buttstein' with the seed-probed FIRE_THRESH 0.2 default", () => {
    const engine = createButtsteinEngine();
    expect(engine.id).toBe('buttstein');
    expect(engine.tweaks).toEqual(BUTTSTEIN_DEFAULTS);
    expect(engine.tweaks.FIRE_THRESH).toBe(0.2);
  });
...
describe('ButtsteinBrain — invariants', () => {
  ...
  it('engaged aim has magnitude AIM_DIST (within rounding) and rescales with the tweak', () => {
    ...
    createButtsteinEngine().createBrain().tick(1, makeCtx(world));
    ...
    createButtsteinEngine({ AIM_DIST: 100 }).createBrain().tick(1, makeCtx(scaled));
  ...
  it('an empty mag forces reload; an in-progress reload does not double-trigger', () => {
    const world = makeWorld(DUEL);
    createButtsteinEngine({ RELOAD_THRESH: 1 })
      .createBrain()
...
describe('ButtsteinBrain — the v3 spray-heat sense', () => {
  const engage = (
    over: Partial<Pick<BotEngineContext, 'sprayHeatOf'>> = {},
  ): string => {
    const world = makeWorld(DUEL);
    createButtsteinEngine().createBrain().tick(1, makeCtx(world, over));
    return snap(world, 1);
  };
