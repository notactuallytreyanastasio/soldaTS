...
const NONEULER_STYLE: ReadonlySet<number> = new Set([
...
const COLLIDABLE_STYLE: ReadonlySet<number> = new Set([
 ...
 */
export function configureSparkParts(parts: ParticleSystem): void {
  parts.timeStep = 1;
 ...
 */
export function killSpark(world: World, sparkIndex: number): void {
 ...
 */
function checkOutOfBounds(world: World, sparkIndex: number): void {
  const map = world.map;
  const sparkParts = world.sparkParts;
 ...
 */
function checkMapCollision(world: World, sparkIndex: number, x: number, y: number): boolean {
  const map = world.map;
  const sparkParts = world.sparkParts;
  if (map === null || sparkParts === null) {
    return false;
  ...
  const pos: Vec2 = vec2(f(x - 8), f(y - 1));
  ...
  for (const w of indices) {
    ...
    if (
      polyType === 1 || // POLY_TYPE_ONLY_BULLETS
    ...
    const vel: Vec2 = vec2(sparkParts.velocityX[num]!, sparkParts.velocityY[num]!);
 ...
 */
function runStyleCollision(world: World, spark: Spark, perp: Vec2, pos: Vec2): void {
...
function killSparkRecord(world: World, spark: Spark): void {
  spark.active = false;
  spark.style = 0;
  if (spark.num > 0 && world.sparkParts !== null) {
    world.sparkParts.active[spark.num] = false;
 ...
 */
export function updateSpark(world: World, sparkIndex: number): void {
  const sparkParts = world.sparkParts;
  if (sparkParts === null) {
    return;
  }
