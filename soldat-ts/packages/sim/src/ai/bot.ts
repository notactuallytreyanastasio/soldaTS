...
import type { WaypointGraph, Waypoint } from './waypoints';
...
// ===========================================================================
// BotState — PORT: shared/mechanics/Sprites.pas TBotData (subset). Folded out of
// the Sprite record because entities/types.ts intentionally OMITS Brain:TBotData
// (see entities/types.ts:47). Each field below maps to a Sprite.Brain.* read in
// AI.pas; fields not yet needed by the ported decisions are noted as TODO.
// ===========================================================================
...
export interface BotState {
  ...
  // goThing: boolean;          // Brain.GoThing — currently chasing a Thing/flag
  // pathNum: number;           // Brain.PathNum — team/escape path selector
  // grenadeFreq, chatFreq, favWeapon, deadKill, onePlaceCount, fallSave,
  // waypointTimeoutCounter ... (AI.pas grenade/chat/camp/timeout machinery)
}

/** Allocate a fresh brain. `accuracy`/`weaponSpeed` default to a mid bot. */
export function createBotState(
  overrides: Partial<BotState> = {},
): BotState {
  return {
    targetNum: 0,
    currentWaypoint: 0,
 ...
 */
function waypointById(graph: WaypointGraph, id: number): Waypoint | undefined {
...
function spritePos(world: World, i: number): { x: number; y: number } {
...
function spriteVel(world: World, i: number): { x: number; y: number } {
  const parts = world.spriteParts;
  if (parts === null) return { x: 0, y: 0 };
...
export function hasLineOfSight(
  ...
  const solid = (polyType: number): boolean =>
 ...
 */
export function findTarget(world: World, selfNum: number): number {
  ...
  for (let i = 1; i < world.sprites.length; i++) {
    ...
    if (!hasLineOfSight(world, lookPoint, startPoint)) continue;
 ...
 */
export function simpleDecision(
  world: World,
  selfNum: number,
  brain: BotState,
): void {
  ...
  if (distX < DIST_FAR) {
    control.mouseAimY = Math.round(
      f(leadY - f(f(f(0.5 * distX) / speed)) + accJitter),
    );
  } else {
    control.mouseAimY = Math.round(
      f(leadY - f(f(f(1.75 * distX) / speed)) + accJitter),
    );
  }
}
 ...
 */
export function navigateWaypoints(
  world: World,
  selfNum: number,
  brain: BotState,
  graph: WaypointGraph,
  ...
  if (wp !== undefined && wp.active) {
    if (wp.x > m.x) control.right = true;
    if (wp.x < m.x) control.left = true;
    // Screen-space y is down-positive: a smaller y is higher. Rise toward it.
    if (wp.y < m.y) control.up = true;
 ...
 */
export function freeControls(control: Control): void {
  control.left = false;
  control.right = false;
  control.up = false;
  control.down = false;
