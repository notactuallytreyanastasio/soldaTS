...
import type { ModGameState, PlayerState } from './api';
 ...
 */
function makeWorld(): { world: World; gameState: ModGameState } {
  ...
  if (s1 === undefined || s2 === undefined) throw new Error('sprite slots missing');
  ...
  const players: (PlayerState | undefined)[] = new Array<PlayerState | undefined>(
    MAX_PLAYERS + 1,
  ).fill(undefined);
  players[1] = { name: 'Alice', team: 1, kills: 5, deaths: 1 };
  players[2] = { name: 'Bob', team: 2, kills: 2, deaths: 3 };

  const gameState: ModGameState = {
    players,
    currentMap: 'ctf_Ash',
    nextMap: 'ctf_Run',
    scoreLimit: 10,
...
describe('createScriptApi — ScriptPlayer reflects the live World', () => {
  ...
  it('reflects sim mutations live (no caching)', () => {
    ...
    const s1 = world.sprites[1];
    if (s1 === undefined) throw new Error('missing');
    s1.health = 42;
    expect(p1.health).toBe(42);

    if (world.spriteParts === null) throw new Error('missing parts');
...
describe('createScriptApi — Players iteration skips inactive', () => {
  ...
  it('Active list updates when a sprite deactivates', () => {
    ...
    const api = createScriptApi(world, gameState);
    const s2 = world.sprites[2];
    if (s2 === undefined) throw new Error('missing');
    s2.active = false;
