...
import type { HandlerErrorInfo } from './host.js';
import { createScriptApi } from './api.js';
import type { ScriptApi, ScriptPlayer } from './api.js';
import { createDoubleDamageMod, type SampleModLog } from './sample-mod.js';
...
describe('ScriptHost OnPlayerDamage cascade', () => {
  ...
  it('a non-number cascade return is ignored (damage carries forward)', () => {
    ...
    host.loadMod((ctx) => {
      ...
      ctx.on('OnPlayerDamage', (() => 'oops') as unknown as (
        v: ScriptPlayer,
        s: ScriptPlayer,
        d: number,
      ) => number);
