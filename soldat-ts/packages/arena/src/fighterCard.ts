 ...
 */
export function validateCard(raw: unknown): {
  card: FighterCard;
  ...
  const c = raw as Partial<FighterCard>;
  ...
  if (typeof c.engine !== 'string' || !engineIds().includes(c.engine)) {
    throw new Error(
      `fighter card: engine must be one of [${engineIds().join(', ')}]`,
    );
  }
  ...
  for (const [k, v] of Object.entries(tweaks)) {
    if (!(k in defaults)) {
      throw new Error(
        `fighter card: unknown knob '${k}' for ${c.engine} (knobs: ${Object.keys(defaults).join(', ')})`,
      );
    }
...
export function tweaksToParam(tweaks: Record<string, number>): string {
