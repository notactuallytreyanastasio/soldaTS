...
import type { World, Sprite, ParticleSystem, Control } from "@soldat/sim";
import {
  POS_STAND,
  POS_CROUCH,
  POS_PRONE,
...
function postureFromPosition(position: number): Posture {
  switch (position) {
    case POS_PRONE:
...
function positionFromPosture(posture: Posture): number {
  switch (posture) {
    case Posture.Prone:
      return POS_PRONE;
...
function buttonsFromControl(c: Control): Buttons {
  return {
    left: c.left,
    right: c.right,
    up: c.up,
...
function writeButtonsToControl(c: Control, b: Buttons): void {
  c.left = b.left;
  c.right = b.right;
  c.up = b.up;
  c.down = b.down;
...
function readPos(parts: ParticleSystem, num: number): Vec2 {
  return { x: parts.posX[num] ?? 0, y: parts.posY[num] ?? 0 };
}

function readVelocity(parts: ParticleSystem, num: number): Vec2 {
 ...
 */
function writePos(parts: ParticleSystem, num: number, pos: Vec2): void {
  parts.posX[num] = pos.x;
  parts.posY[num] = pos.y;
}

function writeVelocity(parts: ParticleSystem, num: number, vel: Vec2): void {
...
function aimOf(sprite: Sprite): MouseAim {
  return { x: sprite.control.mouseAimX, y: sprite.control.mouseAimY };
}

function weaponOf(sprite: Sprite): WeaponState {
  return {
...
function cosmeticsOf(sprite: Sprite): Cosmetics {
  return {
 ...
 */
export function captureSpriteSnapshotOne(
  sprite: Sprite,
...
export function captureSpriteSnapshot(
  world: World,
  serverTick: number = world.serverTickCounter,
): Message[] {
 ...
 */
export function applySpriteSnapshot(
  world: World,
  msg: SpriteSnapshotFull | Extract<Message, { kind: "spriteSnapshot" }>,
): void {
  ...
  if (snap.kind !== "full") {
    ...
    throw new Error(
      `applySpriteSnapshot: expected a full snapshot, got "${snap.kind}"`,
    );
  }
...
function aimEqual(a: MouseAim, b: MouseAim): boolean {
  return a.x === b.x && a.y === b.y;
}

function buttonsEqual(a: Buttons, b: Buttons): boolean {
  return (
    a.left === b.left &&
    a.right === b.right &&
    a.up === b.up &&
 ...
 */
export function diffSpriteSnapshot(
  prev: SpriteSnapshotFull,
  curr: SpriteSnapshotFull,
): SpriteSnapshotDelta {
  const delta: SpriteSnapshotDelta = {
    kind: "delta",
    num: curr.num,
  ...
  if (
    prev.weapon.weaponNum !== curr.weapon.weaponNum ||
 ...
 */
export function applySpriteDelta(
  base: SpriteSnapshotFull,
  delta: SpriteSnapshotDelta,
): SpriteSnapshotFull {
  const weapon: WeaponState =
    delta.weapon !== undefined
      ? {
